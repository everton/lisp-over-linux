...
[[./media/screenshots/screenshot-2026-08-09.png]]
...
[[./media/screenshots/browser-2026-08-10.png]]

/The code browser running as PID 1 on the bare framebuffer (=b= from the
supervisor menu). It opens on the *module index* — every definition grouped by
the file it came from, with the selected module's contents kind-coloured beside
it. That is the only root that shows the whole system: the toolkit lives in its
own =lol.*= packages, so a single-package root cannot list it at all./
...
locals. If one flat alphabetical list is what you actually want, =l= at the
supervisor menu opens the =CL-USER= package view instead.
...
#+begin_src
  ...
  check-links.sh           verify every [[file:...]] link in the .org docs resolves
  refresh-screenshot.sh    re-capture the screenshots (headless QEMU, driven over QMP)
  loc.sh                   lines of code/prose, by language
